...
<link href="https://fonts.googleapis.com/css2?family=Montserrat&display=swap" rel="stylesheet">
    ...
    <body>
          <div class="barraroja">
           ...
           <a href="usuario.jsp"> <button class="regresar" style="/* color: white; */position: relative;
    top: 100px;
    /* left: 500px; */height: 40px;
    color: white;
    font-size: 20px;
    left: 1700px;
    text-transform: uppercase;
    font-weight: bold;">Regresar</button></a>
       </div>
                <% 
           Connection con = null;
            Statement set = null;
                     ...
                     System.out.println("Esta es la sesion "+String.valueOf(session.getAttribute("id")));
                        try{
                            ...
                            String q = "select id_usuario,intentos,buenas from oportunidades  where buenas='"+valos+"' and id_usuario='"+session.getAttribute("id")+"'";
                             ...
                             String axd ="SELECT B.* FROM publiguardada A , perdido B  WHERE A.id_publi =B.idperdido ";
                            set = con.createStatement();
                            rs = set.executeQuery(q);
                             rssa = set.executeQuery(axd);
                            while(rssa.next()){
                            
                %>
       <div class="rectangulo" id="resultado">  
                     <h3> <p class="encontrado">---------------------PERDIDO--------------------</p></h3> 
                    <h1><p class="titupubli">Titulo:<%=rssa.getString("titulo")%></p></h1>
                   <br>
                   <br>
                   <span style="right: 5px; font-size: large; font:bold; position: relative; top: -99px;">&nbsp;&nbsp;&nbsp;Descripcion:<%=rssa.getString("queperdio")%></span>
                    <br>
                    <span class="fecha">&nbsp;&nbsp;&nbsp;Horario de Entrega: <%=rssa.getString("horario")%></span>
                    <p class="titupublis">&nbsp;&nbsp;&nbsp;Lugar de Entrega:<%=rssa.getString("lugarentrega")%></p>
                    <button method ="post" class="marcarentrega" id="boton" style="color: white" value="<%=rssa.getString("idperdido")%>" onclick="valo(this.value)">ENTREGADO</button>
                        <button method ="post" class="marcarnoentrega" id="boton" style="color: white" value="<%=rssa.getString("idperdido")%>" onclick="valiras(this.value)">NO ENTREGADO</button>
                   </div>
                    <br>
                    <br>
                <%     
                ...
                %>
                   ...
                   <span style="right: 5px; font-size: large; font:bold; position: relative; top: -99px;">&nbsp;&nbsp;&nbsp;Descripcion:<%=rss.getString("queperdio")%></span>
                    ...
                    <button method ="post" class="marcarentrega" id="boton" style="color: white" value="<%=rss.getString("id_publi")%>" onclick="valor(this.value)">ENTREGADO</button>
                        <button method ="post" class="marcarnoentrega" id="boton" style="color: white" value="<%=rss.getString("id_publi")%>" onclick="valoras(this.value)">NO ENTREGADO</button>
                   </div>
                    <br>
                    <br>
                    <%     }
        ...
        function valoras(val)
                 {
                   ...
                   var mensaje =btoa(resultado.toString());
                   console.log(mensaje + key + " epoc " + now);
                   window.location.replace("noentregado.jsp?id="+mensaje+"&log="+key);
                   return false;
                 }
        </script>
         <script>  
        function valo(val)
                 {
                   ...
                   var mensaje =btoa(resultado.toString());
                   console.log(mensaje + key + " epoc " + now);
                   window.location.replace("entregado-Ne-Perdido.jsp?id="+mensaje+"&log="+key);
                   return false;
                 }
        </script>
           <script>  
        function valiras(val)
                 {
                   const now = Date.now();
                   var resultado = (parseInt(val) / now);
                   var key = btoa(now.toString());
                   var mensaje =btoa(resultado.toString());
                   console.log(mensaje + key + " epoc " + now);
                   window.location.replace("No-entregado-Perdido.jsp?id="+mensaje+"&log="+key);
                   return false;
                 }
        </script>
</html>
